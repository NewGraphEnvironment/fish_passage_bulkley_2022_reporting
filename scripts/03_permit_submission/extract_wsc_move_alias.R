...
WHERE a.watershed_group_code IN ('BULK')
ORDER BY a.stream_crossing_id, b.match_type;"
)  %>%
  filter(stream_crossing_id %in% (pscis_phase2 %>% pull(pscis_crossing_id)))

# define our crs
##now we need to join to our habitat_confirmations sheet in the right order so we can copy and paste into the spreadsheet
wsc_join <- left_join(
  hab_loc %>%
...
permit_id = 'SM22-770368'

file.copy(from = 'data/habitat_confirmations.xls',
          to = paste0('C:/Users/matwi/OneDrive/Projects/repo/fish_passage_bulkley_2022_reporting/permit_submission/', permit_id, '_data.xls'))
